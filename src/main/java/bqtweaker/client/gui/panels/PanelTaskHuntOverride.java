...
public class PanelTaskHuntOverride extends CanvasEmpty
{
    private final TaskHunt task;
    
    public PanelTaskHuntOverride(IGuiRect rect, TaskHunt task)
    {
        super(rect);
        this.task = task;
    }
    ...
    public void initPanel()
    {
        ...
        if(target != null) this.addPanel(new PanelEntityPreviewOverride(new GuiTransform(GuiAlign.FULL_BOX, new GuiPadding(0, 16, 0, 0), 0), target).setRotationDriven(new ValueFuncIO<>(() -> 15F), new ValueFuncIO<>(() -> (float)(Minecraft.getSystemTime()%30000L / 30000D * 360D))));
    }
}
